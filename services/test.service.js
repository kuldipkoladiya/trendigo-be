...
export async function createTest(body, options = {}) {
...
export async function aggregateTest(query) {
  const test = await Test.aggregate(query);
  return test;
}

export async function aggregateTestWithPagination(query, options = {}) {
  const aggregate = Test.aggregate();
  query.map((obj) => {
    aggregate._pipeline.push(obj);
  });
  const test = await Test.aggregatePaginate(aggregate, options);
  return test;
}
