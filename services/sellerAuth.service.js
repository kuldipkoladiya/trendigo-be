...
import { emailService, tokenService } from './index';
...
export const register = async (body) => {
  ...
  if (email) {
    const verifyEmailToken = await tokenService.generateVerifyEmailToken(seller, 'seller');
 ...
 * Verify OTP
 */
export const verifyOtp = async ({ email, mobileNumber }) => {
 ...
 */
export const loginUserWithEmailAndPassword = async (email) => {
  if (!email) {
  ...
  if (!user.isEmailVerified) {
    throw new ApiError(httpStatus.BAD_REQUEST, 'Please check your email and verify it to continue logging in to the app');
  }
